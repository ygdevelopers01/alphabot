# ================================================================
#   ALPHA TRADING BOT v3.0 — BYBIT DEMO
#   Single file — Just run: python3 alphabot.py
#   Exchange: Bybit Demo Trading
#   Strategies: EMA 9/20 | EMA Touch | RSI | Swing
# ================================================================
...
# Bybit API URLs
BYBIT_DEMO_URL = "https://api-demo.bybit.com"   # Demo trading URL
BYBIT_LIVE_URL = "https://api.bybit.com"         # Live trading URL (use when ready)

def BASE_URL():
    return BYBIT_DEMO_URL if TRADING_MODE == "demo" else BYBIT_LIVE_URL
...
class BybitAPI:
    ...
    def _get(self, path, params=None):
        ...
        try:
            ...
            if data.get("retCode") != 0:
                log.error(f"Bybit API error: {data.get('retMsg')} | Code: {data.get('retCode')}")
                return None
            return data
    ...
    def set_leverage(self, symbol, leverage):
        data = {"category":"linear","symbol":symbol,
                "buyLeverage":str(leverage),"sellLeverage":str(leverage)}
        return self._post("/v5/position/set-leverage", data)

    def place_order(self, symbol, side, qty, stop_loss, take_profit, category="linear"):
        """Place market order with auto SL and TP."""
        data = {
            "category":    category,
            "symbol":      symbol,
        ...
        if r:
            log.info(f"Order placed: {symbol} {side} {qty} | SL:{stop_loss} TP:{take_profit}")
            return r.get("result", {})
        return None
    ...
    def close_position(self, symbol):
        ...
        data = {"category":"linear","symbol":symbol,"side":side,
                "orderType":"Market","qty":size,"reduceOnly":True,"timeInForce":"GTC"}
    ...
    def get_order_history(self, symbol=None, limit=50):
        ...
        r = self._get("/v5/order/history", params)
        if r and r.get("result"):
            return r["result"].get("list", [])
        return []
...
def strategy_ema_touch(df):
    ...
    bb  = c["is_bull"] and c["body"] > c["atr"]*0.3 and c["close"] > e20
    sb  = c["is_bear"] and c["body"] > c["atr"]*0.3 and c["close"] < e20
...
def analyze_one_tf(df):
    ...
    c = df.iloc[-1]
    base = {"atr":float(c["atr"]),"close":float(c["close"]),"ema9":float(c["ema9"]),"ema20":float(c["ema20"]),"rsi":float(c["rsi"])}
...
def get_scalp_signal(tf_data):
    ...
    return {"signal":"NONE","reason":f"{len(nn)}/3 scalp TFs"}

def get_swing_signal(tf_data):
    dfs = {}
    for tf in ["1w","1d","4h","1h"]:
        df = tf_data.get(tf)
        if df is not None and len(df)>=10: dfs[tf] = add_indicators(df)
...
class TradeState:
    ...
    def has_active(self, symbol): return symbol in self.active_trades
    def get_active(self, symbol): return self.active_trades.get(symbol)
    ...
    def close_trade(self, symbol, exit_price, reason):
        ...
        trade.update({"exit_price":exit_price,"exit_reason":reason,
                      ...
                      "pnl":round(pts*size,2),"status":"CLOSED"})
        with self.lock:
...
class Telegram:
    ...
    def trade_opened(self, t):
        ...
        self.send(f"{emoji} *TRADE OPENED* | {t['symbol']}\n"
                  ...
                  f"Strategy: {t.get('strategy','—')} | Strength: {t.get('strength',0)}/100")
...
class PositionMonitor:
    ...
    def check_all(self):
        ...
        for symbol, trade in active.items():
            ...
            if live_size == 0:
                self._finalize(symbol, trade, float(trade["entry_price"]), "CLOSED_ON_EXCHANGE")
                continue
...
class AlphaBot:
    def __init__(self):
        log.info("="*50)
        log.info("  ALPHA BOT v3.0 — BYBIT DEMO")
        log.info("="*50)
        self.api     = BybitAPI()
    ...
    def _analyze(self, symbol):
        ...
        atr    = float(curr["atr"])
        sltp   = calc_sltp(symbol, chosen["signal"], entry, atr, chosen.get("trade_type","INTRADAY"))
        ...
        trade = {
            ...
            "entry_price":entry,"stop_loss":sltp["stop_loss"],"take_profit":sltp["take_profit"],
            "size":cfg["lot_size"],"rr":sltp["rr"],"strategy":chosen.get("entry","MULTI_TF"),
            "strength":chosen.get("strength",0),"entry_time":datetime.now().isoformat(),
            "entry_ts":time.time(),"order_id":order.get("orderId",""),
        }
        self.state.open_trade(trade)
        self.tg.trade_opened(trade)
        log.info(f"✅ Trade opened: {symbol} {chosen['signal']} @ {entry:.2f}")
    ...
    def get_dashboard_data(self):
        ...
        return {
            ...
            "mode":          "BYBIT " + TRADING_MODE.upper(),
        }

# ================================================================
#   SECTION 11 — DASHBOARD (unchanged from before)
# ================================================================
...
LOGIN_HTML="""<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>ALPHA Bot Login</title>
<link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;600;700;800&family=JetBrains+Mono:wght@400;700&display=swap" rel="stylesheet">
<style>*{margin:0;padding:0;box-sizing:border-box}body{background:#03060f;color:#e8f0fe;font-family:'Inter',sans-serif;min-height:100vh;display:flex;align-items:center;justify-content:center;background-image:linear-gradient(rgba(0,212,170,.03) 1px,transparent 1px),linear-gradient(90deg,rgba(0,212,170,.03) 1px,transparent 1px);background-size:40px 40px}.card{background:linear-gradient(145deg,#070d1c,#0c1526);border:1px solid rgba(0,212,170,.2);border-radius:20px;padding:52px 48px 44px;width:420px;position:relative;overflow:hidden;box-shadow:0 50px 100px rgba(0,0,0,.7)}.card::before{content:'';position:absolute;top:0;left:0;right:0;height:2px;background:linear-gradient(90deg,transparent,#00d4aa,#00aaff,transparent)}.top{text-align:center;margin-bottom:36px}.icon{width:70px;height:70px;background:rgba(0,212,170,.1);border:1px solid rgba(0,212,170,.25);border-radius:18px;display:flex;align-items:center;justify-content:center;font-size:34px;margin:0 auto 18px;animation:float 3s ease-in-out infinite}@keyframes float{0%,100%{transform:translateY(0)}50%{transform:translateY(-8px)}}h1{font-size:26px;font-weight:800;background:linear-gradient(135deg,#00d4aa,#00aaff);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}p{font-size:12px;color:#4a6080;letter-spacing:1px;text-transform:uppercase;margin-top:6px}.f{margin:16px 0}.f label{display:block;font-size:11px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;color:#4a6080;margin-bottom:8px}.f input{width:100%;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:10px;padding:13px 16px;color:#e8f0fe;font-family:'JetBrains Mono',monospace;font-size:14px;outline:none;transition:all .2s}.f input:focus{border-color:#00d4aa;background:rgba(0,212,170,.04)}.btn{width:100%;background:linear-gradient(135deg,#00d4aa,#00aaff);border:none;border-radius:10px;padding:14px;color:#03060f;font-size:15px;font-weight:700;cursor:pointer;margin-top:12px;transition:all .2s}.btn:hover{transform:translateY(-2px);box-shadow:0 10px 30px rgba(0,212,170,.3)}.err{background:rgba(240,54,90,.1);border:1px solid rgba(240,54,90,.25);border-radius:8px;padding:10px 14px;color:#f87191;font-size:13px;margin-bottom:14px;text-align:center;display:none}.hint{margin-top:18px;text-align:center;background:rgba(0,212,170,.05);border:1px solid rgba(0,212,170,.1);border-radius:10px;padding:12px;font-size:12px;color:#4a6080;line-height:1.9}.hint b{color:#00d4aa;font-family:'JetBrains Mono',monospace}</style></head><body>
<div class="card"><div class="top"><div class="icon">🤖</div><h1>ALPHA Trading Bot</h1><p>Bybit Demo • 24/7 Auto Trading</p></div>
{% if error %}<div class="err" style="display:block">❌ {{ error }}</div>{% endif %}
<form method="POST" action="/login"><div class="f"><label>Username</label><input type="text" name="username" placeholder="@ALPHA" required autofocus></div>
<div class="f"><label>Password</label><input type="password" name="password" placeholder="••••••••" required></div>
<button type="submit" class="btn">🔐 LOGIN TO BOT</button></form>
<div class="hint">Login → Username: <b>@ALPHA</b> &nbsp;/&nbsp; Password: <b>@ALPHA01</b></div></div></body></html>"""

DASH_HTML="""<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>ALPHA Trading Bot</title>
<link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800;900&family=JetBrains+Mono:wght@400;500;700&display=swap" rel="stylesheet">
<script src="https://cdnjs.cloudflare.com/ajax/libs/socket.io/4.7.2/socket.io.min.js"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/Chart.js/4.4.1/chart.umd.js"></script>
<style>:root{--bg:#03060f;--s1:#070d1c;--s2:#0c1526;--s3:#101d31;--ac:#00d4aa;--bl:#00aaff;--rd:#f0365a;--yl:#f5a623;--tx:#e8f0fe;--mt:#4a6080;--bd:rgba(0,212,170,0.1);--fn:'Inter',sans-serif;--mn:'JetBrains Mono',monospace}*{margin:0;padding:0;box-sizing:border-box}body{background:var(--bg);color:var(--tx);font-family:var(--fn);overflow-x:hidden;font-size:14px;background-image:linear-gradient(rgba(0,212,170,.02) 1px,transparent 1px),linear-gradient(90deg,rgba(0,212,170,.02) 1px,transparent 1px);background-size:40px 40px}body::after{content:'';position:fixed;inset:0;background:radial-gradient(ellipse 900px 700px at 10% 20%,rgba(0,212,170,.04),transparent 60%),radial-gradient(ellipse 700px 600px at 90% 80%,rgba(0,170,255,.04),transparent 60%);pointer-events:none;z-index:0}
...
.brand{display:flex;align-items:center;gap:10px}.bicon{width:32px;height:32px;background:rgba(0,212,170,.15);border:1px solid rgba(0,212,170,.3);border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:16px}.bname{font-size:17px;font-weight:800;background:linear-gradient(135deg,var(--ac),var(--bl));-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}
.npairs{display:flex;gap:3px;background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.06);border-radius:8px;padding:3px}.np{padding:5px 13px;border-radius:6px;font-size:12px;font-weight:600;color:var(--mt);cursor:pointer;transition:all .15s;font-family:var(--mn)}.np.on{background:var(--s3);color:var(--ac)}
.nav-r{display:flex;align-items:center;gap:12px}.bpill{display:flex;align-items:center;gap:7px;border-radius:20px;padding:5px 13px;font-size:11px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;font-family:var(--mn)}.pr{background:rgba(0,212,170,.1);border:1px solid rgba(0,212,170,.25);color:var(--ac)}.ps{background:rgba(240,54,90,.1);border:1px solid rgba(240,54,90,.25);color:var(--rd)}.pi{background:rgba(245,166,35,.1);border:1px solid rgba(245,166,35,.25);color:var(--yl)}.dot{width:6px;height:6px;border-radius:50%;background:currentColor}.dot.pulse{animation:hb 1.8s infinite}@keyframes hb{0%,100%{transform:scale(1);opacity:1}50%{transform:scale(1.5);opacity:.4}}
.clk{font-family:var(--mn);font-size:12px;color:var(--mt)}.lo{background:none;border:1px solid rgba(240,54,90,.2);border-radius:6px;padding:5px 12px;color:var(--rd);font-size:11px;cursor:pointer;font-family:var(--fn);font-weight:600}.lo:hover{background:rgba(240,54,90,.08)}
.main{max-width:1800px;margin:0 auto;padding:18px 24px;position:relative;z-index:1}
.prices{display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin-bottom:18px}.pc{background:linear-gradient(145deg,var(--s1),var(--s2));border:1px solid rgba(255,255,255,.06);border-radius:12px;padding:18px 22px;display:flex;justify-content:space-between;align-items:center;position:relative;overflow:hidden}.pc::after{content:'';position:absolute;top:0;left:0;right:0;height:1px;background:linear-gradient(90deg,transparent,var(--c),transparent);opacity:.4}.pc.btc{--c:#f7931a}.pc.eth{--c:#627eea}.pc.sol{--c:#9945ff}
.pc-sym{font-size:11px;font-weight:700;letter-spacing:2px;color:var(--mt);text-transform:uppercase;margin-bottom:5px}.pc-price{font-family:var(--mn);font-size:28px;font-weight:700;letter-spacing:-1px}.pc-chg{font-family:var(--mn);font-size:12px;margin-top:4px}.cp{color:var(--ac)}.cn{color:var(--rd)}.pl{text-align:right}.pl-lb{font-size:10px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;color:var(--mt)}.pl-vl{font-family:var(--mn);font-size:14px;font-weight:700;color:var(--c);margin-top:4px}
.stats{display:grid;grid-template-columns:repeat(6,1fr);gap:12px;margin-bottom:18px}.sc{background:linear-gradient(145deg,var(--s1),var(--s2));border:1px solid rgba(255,255,255,.06);border-radius:12px;padding:16px 18px;position:relative;overflow:hidden}.sc::before{content:'';position:absolute;bottom:0;left:0;right:0;height:2px;background:var(--cc,var(--ac));opacity:0;transition:opacity .3s}.sc:hover::before{opacity:.6}.sl{font-size:10px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;color:var(--mt);margin-bottom:10px}.sv{font-family:var(--mn);font-size:24px;font-weight:700;line-height:1;margin-bottom:4px}.ss{font-size:11px;color:var(--mt)}.vg{color:var(--ac)}.vr{color:var(--rd)}.vy{color:var(--yl)}.vb{color:var(--bl)}.vp{color:#9945ff}.vm{color:var(--mt)}
.mgrid{display:grid;grid-template-columns:1fr 380px;gap:18px;margin-bottom:18px}.cp2{background:linear-gradient(145deg,var(--s1),var(--s2));border:1px solid rgba(255,255,255,.06);border-radius:14px;overflow:hidden}
.ph{display:flex;align-items:center;justify-content:space-between;padding:14px 18px;border-bottom:1px solid rgba(255,255,255,.05);background:rgba(0,0,0,.15)}.ph-t{font-size:13px;font-weight:700;display:flex;align-items:center;gap:8px}.ld{width:7px;height:7px;border-radius:50%;background:var(--ac);animation:hb 1.5s infinite}.tfs{display:flex;gap:3px}.tf{background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.06);border-radius:5px;padding:4px 10px;font-family:var(--mn);font-size:11px;color:var(--mt);cursor:pointer;transition:all .15s}.tf.on{background:rgba(0,212,170,.1);border-color:rgba(0,212,170,.3);color:var(--ac)}
.ca{height:490px;position:relative;background:var(--s1)}.ca iframe{width:100%;height:100%;border:none;display:block;position:absolute;top:0;left:0}.cl{position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;background:var(--s1)}.sp{width:38px;height:38px;border:2px solid rgba(0,212,170,.15);border-top-color:var(--ac);border-radius:50%;animation:spin .8s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}.cl p{font-family:var(--mn);font-size:12px;color:var(--mt)}
.rp{display:flex;flex-direction:column;gap:14px}.sbx{background:linear-gradient(145deg,var(--s1),var(--s2));border:1px solid rgba(255,255,255,.06);border-radius:14px;overflow:hidden}
.tfm{display:grid;grid-template-columns:repeat(3,1fr);gap:6px;padding:12px}.tfc{background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.05);border-radius:8px;padding:9px 6px;text-align:center;transition:all .4s}.tfc.bull{background:rgba(0,212,170,.07);border-color:rgba(0,212,170,.2)}.tfc.bear{background:rgba(240,54,90,.07);border-color:rgba(240,54,90,.2)}.tfc.wait{background:rgba(245,166,35,.05);border-color:rgba(245,166,35,.15)}.tn{font-size:9px;font-weight:700;letter-spacing:1.5px;color:var(--mt);text-transform:uppercase;margin-bottom:3px}.ts2{font-size:14px;font-weight:800;font-family:var(--mn)}.tfc.bull .ts2{color:var(--ac)}.tfc.bear .ts2{color:var(--rd)}.tfc.wait .ts2{color:var(--yl)}.tst{font-size:9px;font-family:var(--mn);color:var(--mt);margin-top:2px}
.atb{padding:0 12px 12px}.ate{border:1px dashed rgba(255,255,255,.06);border-radius:10px;padding:20px;text-align:center;font-size:12px;color:var(--mt);line-height:1.8}
.atcard{background:var(--s3);border:1px solid rgba(0,212,170,.15);border-radius:10px;padding:14px;position:relative;overflow:hidden}.atcard::before{content:'';position:absolute;top:0;left:0;right:0;height:2px;background:linear-gradient(90deg,var(--ac),var(--bl))}.at-top{display:flex;align-items:center;justify-content:space-between;margin-bottom:12px}.at-sym{font-size:20px;font-weight:800}.at-badge{border-radius:20px;padding:4px 12px;font-size:11px;font-weight:700;letter-spacing:1px}.atl{background:rgba(0,212,170,.15);color:var(--ac);border:1px solid rgba(0,212,170,.25)}.ats{background:rgba(240,54,90,.15);color:var(--rd);border:1px solid rgba(240,54,90,.25)}.at-cells{display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px}.atcell{background:rgba(0,0,0,.2);border-radius:7px;padding:8px;text-align:center}.atcell label{display:block;font-size:9px;font-weight:700;letter-spacing:1.2px;text-transform:uppercase;color:var(--mt);margin-bottom:3px}.atcell .v{font-family:var(--mn);font-size:13px;font-weight:700}.at-pb-w{height:3px;background:rgba(255,255,255,.06);border-radius:2px;margin-top:10px;overflow:hidden}.at-pb{height:100%;border-radius:2px;background:linear-gradient(90deg,var(--ac),var(--bl));transition:width .6s}
.srows{padding:0 12px 12px;display:flex;flex-direction:column;gap:5px}.srow{display:flex;align-items:center;justify-content:space-between;background:rgba(255,255,255,.02);border:1px solid rgba(255,255,255,.04);border-radius:7px;padding:7px 10px}.sr-n{font-size:12px;color:var(--tx)}.tag{display:inline-block;padding:2px 8px;border-radius:10px;font-size:9px;font-weight:700;letter-spacing:.5px;font-family:var(--mn)}.t-sc{background:rgba(245,166,35,.1);color:var(--yl)}.t-bl{background:rgba(0,212,170,.1);color:var(--ac)}.t-br{background:rgba(240,54,90,.1);color:var(--rd)}.t-ok{background:rgba(0,170,255,.1);color:var(--bl)}
.tl{background:rgba(0,212,170,.1);color:var(--ac)}.ts{background:rgba(240,54,90,.1);color:var(--rd)}.tw{background:rgba(0,212,170,.1);color:var(--ac)}.tls{background:rgba(240,54,90,.1);color:var(--red)}.to{background:rgba(245,166,35,.1);color:var(--yl)}
.bgrid{display:grid;grid-template-columns:1fr 1fr;gap:18px;margin-bottom:18px}.eb,.hb{background:linear-gradient(145deg,var(--s1),var(--s2));border:1px solid rgba(255,255,255,.06);border-radius:14px;overflow:hidden}.ew{padding:14px;height:220px;position:relative}
.tw2{overflow-y:auto;max-height:260px}.tbl{width:100%;border-collapse:collapse;font-size:12px}.tbl th{position:sticky;top:0;background:var(--s2);padding:9px 12px;font-size:9px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;color:var(--mt);text-align:left;border-bottom:1px solid rgba(255,255,255,.05)}.tbl td{padding:9px 12px;border-bottom:1px solid rgba(255,255,255,.025)}.tbl tr:hover td{background:rgba(255,255,255,.02)}.mono{font-family:var(--mn);font-size:11px}
.lb-box{background:linear-gradient(145deg,var(--s1),var(--s2));border:1px solid rgba(255,255,255,.06);border-radius:14px;overflow:hidden;margin-bottom:18px}.log-body{padding:10px;max-height:160px;overflow-y:auto;display:flex;flex-direction:column;gap:4px}.li{background:rgba(255,255,255,.02);border-radius:6px;padding:7px 11px;display:flex;align-items:flex-start;gap:9px;border-left:2px solid transparent;font-size:12px}.li.info{border-left-color:var(--bl)}.li.ok{border-left-color:var(--ac)}.li.warn{border-left-color:var(--yl)}.li.err{border-left-color:var(--rd)}.lt{font-family:var(--mn);font-size:10px;color:var(--mt);white-space:nowrap;margin-top:1px;min-width:60px}.lm{color:var(--tx);line-height:1.5}
...
@media(max-width:1200px){.mgrid{grid-template-columns:1fr}.stats{grid-template-columns:repeat(3,1fr)}}@media(max-width:768px){.prices{grid-template-columns:1fr}.stats{grid-template-columns:1fr 1fr}.bgrid{grid-template-columns:1fr}}</style></head><body>
<nav><div class="brand"><div class="bicon">🤖</div><div class="bname">ALPHA BOT</div></div>
<div class="npairs"><div class="np on" onclick="switchPair('BTCUSDT',this)">₿ BTC</div><div class="np" onclick="switchPair('ETHUSDT',this)">Ξ ETH</div><div class="np" onclick="switchPair('SOLUSDT',this)">◎ SOL</div></div>
<div class="nav-r"><div class="clk" id="clk">00:00:00</div><div class="bpill pi" id="bpill"><div class="dot" id="bdot"></div><span id="btxt">LOADING</span></div><div style="font-size:12px;color:var(--mt)">👤 {{ username }}</div><a href="/logout" class="lo">Logout</a></div></nav>
<div class="main">
<div class="prices">
<div class="pc btc"><div><div class="pc-sym">₿ BTC/USDT</div><div class="pc-price" id="pBTC">$—</div><div class="pc-chg" id="cBTC">—</div></div><div class="pl"><div class="pl-lb">Lot Size</div><div class="pl-vl">5 lots</div></div></div>
<div class="pc eth"><div><div class="pc-sym">Ξ ETH/USDT</div><div class="pc-price" id="pETH">$—</div><div class="pc-chg" id="cETH">—</div></div><div class="pl"><div class="pl-lb">Lot Size</div><div class="pl-vl">8 lots</div></div></div>
<div class="pc sol"><div><div class="pc-sym">◎ SOL/USDT</div><div class="pc-price" id="pSOL">$—</div><div class="pc-chg" id="cSOL">—</div></div><div class="pl"><div class="pl-lb">Lot Size</div><div class="pl-vl">3 lots</div></div></div>
</div>
<div class="stats">
...
<div class="sc" style="--cc:var(--bl)"><div class="sl">💼 Exchange</div><div class="sv vm" id="sMode">BYBIT</div><div class="ss">$<span id="sBal">—</span></div></div>
</div>
<div class="mgrid">
<div class="cp2">
<div class="ph"><div class="ph-t"><div class="ld"></div><span id="chartTitle">BTCUSDT — Bybit</span></div>
...
<script>
const sock=io();let cp='BTCUSDT',ct='240',ec=null,pv={},ep=[0],el=['Start'];
...
function upd(d){
  ...
  document.getElementById('sUp').textContent=(st.uptime_hours||0)+'h';document.getElementById('sMode').textContent=d.mode||'BYBIT';document.getElementById('sBal').textContent=(d.balance||0).toFixed(2);
  const px=d.prices||{};
...
function rAT(trades,prices){
  ...
  box.innerHTML=keys.map(sym=>{
    ...
    <div class="at-cells"><div class="atcell"><label>Entry</label><div class="v">$${ep2.toFixed(2)}</div></div><div class="atcell"><label>Mark</label><div class="v">$${px.toFixed(2)}</div></div><div class="atcell"><label>PnL</label><div class="v" style="color:${pts>=0?'var(--ac)':'var(--rd)'}">${pts>=0?'+':''}$${pnl}</div></div>
    <div class="atcell"><label>SL</label><div class="v" style="color:var(--rd)">$${sl.toFixed(2)}</div></div><div class="atcell"><label>TP</label><div class="v" style="color:var(--ac)">$${tp.toFixed(2)}</div></div><div class="atcell"><label>Lots</label><div class="v">${t.size}</div></div></div>
    <div class="at-pb-w"><div class="at-pb" style="width:${prog}%"></div></div></div>`;
  }).join('');
}
...
function initEq(){const ctx=document.getElementById('eqChart').getContext('2d');ec=new Chart(ctx,{type:'line',data:{labels:el,datasets:[{label:'PnL',data:ep,borderColor:'#00d4aa',backgroundColor:'rgba(0,212,170,.06)',borderWidth:2,fill:true,tension:.4,pointRadius:3,pointBackgroundColor:'#00d4aa'}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{display:false},y:{grid:{color:'rgba(255,255,255,.03)'},ticks:{color:'#4a6080',font:{family:'JetBrains Mono',size:10},callback:v=>(v>=0?'+':'')+'$'+v.toFixed(0)}}}}});}
...
function loadChart(pair,tf){
  cp=pair;ct=tf;
  const bybitSym={BTCUSDT:'BINANCE:BTCUSDT',ETHUSDT:'BINANCE:ETHUSDT',SOLUSDT:'BINANCE:SOLUSDT'};
  document.getElementById('chartTitle').textContent=`${pair} — TradingView`;
  const ca=document.getElementById('ca');const old=ca.querySelector('iframe,div:not(#cl)');if(old&&old.id!=='cl')old.remove();document.getElementById('cl').style.display='flex';
  const iframe=document.createElement('iframe');iframe.style.cssText='width:100%;height:100%;border:none;display:block;position:absolute;top:0;left:0;opacity:0;transition:opacity .5s';
  iframe.src='https://s.tradingview.com/widgetembed/?frameElementId=tvf&symbol='+bybitSym[pair]+'&interval='+tf+'&hidesidetoolbar=0&hidetoptoolbar=0&symboledit=0&saveimage=0&toolbarbg=0c1526&studies=EMA%40tv-basicstudies%1FEMA%40tv-basicstudies%1FRSI%40tv-basicstudies&theme=dark&style=1&timezone=Asia%2FKolkata&locale=en';
  let ok=false;iframe.onload=()=>{ok=true;document.getElementById('cl').style.display='none';iframe.style.opacity='1';};setTimeout(()=>{if(!ok){document.getElementById('cl').style.display='none';const d=document.createElement('div');d.style.cssText='position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;background:var(--s1);text-align:center;padding:24px';d.innerHTML=`<div style="font-size:32px">📊</div><a href="https://www.tradingview.com/chart/?symbol=BYBIT:${pair}" target="_blank" style="background:rgba(0,212,170,.12);border:1px solid rgba(0,212,170,.3);border-radius:8px;padding:10px 20px;color:var(--ac);text-decoration:none;font-size:13px">Open ${pair} Chart ↗</a>`;ca.appendChild(d);}},9000);ca.appendChild(iframe);
}
...
window.onload=()=>{buildTFM();buildSrows();initEq();loadChart('BTCUSDT','240');setInterval(animTF,3000);log2('ok','✅ Connected to ALPHA Bot — Bybit Demo Trading');};
</script></body></html>"""
...
║   ALPHA TRADING BOT v3.0 — BYBIT DEMO               ║
║   Pairs: BTCUSDT | ETHUSDT | SOLUSDT                ║
║   RR: 1:2.5 | EMA + RSI + Swing Strategies          ║
╚══════════════════════════════════════════════════════╝
    """)
    if BYBIT_API_KEY=="YOUR_BYBIT_API_KEY_HERE":
        print("⚠️  Add your Bybit API keys in Railway Variables!")
    bot_thread=threading.Thread(target=get_bot().start,daemon=True,name="TradingEngine")
    bot_thread.start()
    time.sleep(2)
